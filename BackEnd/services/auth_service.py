...
class AuthService:
    ...
    def register(self, username: str, password: str, email: str = None, nickname: str = None) -> Dict[str, Any]:
        ...
        try:
            with self._get_db() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    f'''INSERT INTO users (id, username, email, password_hash, nickname, created_at, updated_at)
            ...
            error_msg = str(e)
            if 'username' in error_msg.lower() or 'Duplicate' in error_msg and 'username' in error_msg:
                return {'success': False, 'message': '用户名已存在'}
            elif 'email' in error_msg.lower() or 'Duplicate' in error_msg and 'email' in error_msg:
                return {'success': False, 'message': '邮箱已被注册'}
            return {'success': False, 'message': f'注册失败: {error_msg}'}
    ...
    def login(self, username: str, password: str) -> Dict[str, Any]:
        ...
        try:
            with self._get_db() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    f'SELECT * FROM users WHERE username = {ph} AND is_guest = 0',
                    (username,)
                )
                ...
                cursor.execute(
                    f'''INSERT INTO sessions (id, user_id, token, created_at, expires_at)
    ...
    def create_guest(self) -> Dict[str, Any]:
        ...
        guest_password_hash = hash_password(generate_id())
        ...
        try:
            with self._get_db() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    f'''INSERT INTO users (id, username, password_hash, nickname, is_guest, created_at, updated_at)
                       VALUES ({ph}, {ph}, {ph}, {ph}, 1, {ph}, {ph})''',
                    (guest_id, guest_name, guest_password_hash, '游客用户', now, now)
                )
    ...
    def verify_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with self._get_db() as conn:
                ...
                cursor.execute(
                    f'''SELECT s.*, u.username, u.email, u.nickname, u.avatar, u.is_guest, u.settings
                       FROM sessions s
                       JOIN users u ON s.user_id = u.id
                       WHERE s.token = {ph} AND s.is_active = 1 AND s.expires_at > {ph}''',
                    (token, now)
                )
    ...
    def logout(self, token: str) -> bool:
        ...
        try:
            with self._get_db() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    f'UPDATE sessions SET is_active = 0 WHERE token = {ph}',
                    (token,)
                )
                if self.db_type == 'sqlite':
                    conn.commit()
    ...
    def get_user(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with self._get_db() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    f'SELECT id, username, email, nickname, avatar, is_guest, settings, created_at FROM users WHERE id = {ph}',
                    (user_id,)
                )
                user = cursor.fetchone()
    ...
    def update_user(self, user_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            with self._get_db() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    f'UPDATE users SET {set_clause} WHERE id = {ph}',
                    values
                )
                if self.db_type == 'sqlite':
                    conn.commit()
